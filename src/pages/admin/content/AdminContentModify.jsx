...
import {getContentApi, updateContentApi} from "../../../api/admin/AdminContentApi.js";
import {toast} from "sonner";

export default function AdminContentModify() {
    ...
    // 콘텐츠 수정
    const handleModifyContent = async (e) => {
        e.preventDefault();
        ...
        try {
            await updateContentApi(contentId, formData);

            navigate(`/admin/content/${contentId}`, {
                state: {
                    modifyContentSuccess: true,
                }
            });
        } catch (error) {
        ...
        const fetchData = async () => {
            try {
                ...
                setForm({
                    ...
                    productionCountry: content.productionCountry ?? '',
                    ageRating: content.ageRating ?? '',
                    contentStatus: content.contentStatus ?? '',
                    trailerUrl: content.trailerUrl ?? '',
                    genreIds: content.genres?.map((genre) => genre.genreId) ?? [],
    ...
    useEffect(() => {
        if (!loading && !form) {
            navigate('/admin/content', {
                state: {
                    notExistingContent: true
                }
            });
        }
    }, [loading, form, navigate]);
    ...
    return (
        <>
            <form onSubmit={handleModifyContent}>
                <TitleLabel title={'콘텐츠 수정'} desc={'콘텐츠 정보를 수정합니다.'}>
                            ...
                            <label className="block cursor-pointer">
                                <div className="flex aspect-[2/3] items-center justify-center overflow-hidden
                                    ...
                                    {posterPreview ? (
                                        ...
                                        <img src={`${import.meta.env.VITE_API_SERVER}/uploads/${form.posterUrl}`} alt='포스터 이미지' className="w-full h-full object-cover"/>
                                    )}
                                </div>
